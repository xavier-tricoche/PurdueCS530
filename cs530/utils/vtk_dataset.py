...
def make_spheres(dataset, radius=1,theta_res=12, phi_res=12, scale=False):
    source = vtk.vtkSphereSource(radius=radius, phi_resolution=phi_res, theta_resolution=theta_res)
    glyphs = vtk.vtkGlyph3D(scaling=scale, scale_mode=vtk.VTK_SCALE_BY_SCALAR)
...
def make_arrows(dataset, shaft_radius=0.2, shaft_resolution=12, tip_length=1, tip_radius=0.5, tip_resolution=12, scale=False):
    source = vtk.vtkArrowSource(shaft_radius=shaft_radius, 
                                shaft_resolution=shaft_resolution, 
                                tip_length=tip_length, tip_radius=tip_radius, 
                                tip_resolution=tip_resolution)
    glyphs = vtk.vtkGlyph3D(scaling=scale, scale_mode=vtk.VTK_SCALE_BY_VECTOR)
    glyphs.SetSourceConnection(source.GetOutputPort())
    glyphs.SetInputData(dataset)
    glyphs.Update()
    return glyphs.GetOutput()
...
def clip_polydata(dataset, plane):
    clip = vtk.vtkClipPolyData(generate_clip_scalars=False, generate_clipped_output=True, value=0)
...
def main(number=100):
    ...
    for dim in range(2,4):
        array = np.random.rand(number, dim)
        poly= make_points(array)
        spheres = make_spheres(poly)
        scalars = np.random.rand(number)
        spheres = make_spheres(poly)
        vectors = np.random.rand(number, dim)
        tensors = np.random.rand(number, dim*dim)
        colors = 255.*np.random.rand(number, 3)
        tcoords = np.random.rand(number, 2)
        ...
        mapper2.SetInputData(spheres)
        mapper2.ScalarVisibilityOn()
        actor2 = vtk.vtkActor()
        actor2.SetMapper(mapper2)

        plane = vtk.vtkPlane()
        clipped = clip_polydata(poly, plane)
        clipped_mapper = vtk.vtkDataSetMapper()
        clipped_mapper.SetInputData(clipped)
        clipped_actor = vtk.vtkActor()
        clipped_actor.SetMapper(clipped_mapper)

        mapper3 = vtk.vtkPolyDataMapper()
        mapper3.SetInputData(arrows)
        actor3 = vtk.vtkActor()
